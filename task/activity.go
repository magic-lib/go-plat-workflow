...
type (
	...
	Activity struct {
		...
		activityConfig activityConfig      //内部配置参数
	}
)

func (ac *Activity) getResponseStoreKey(keyPrefix string) string {
	activityName := ""
	if ac.Namespace == "" {
		activityName = ac.Activity
	} else {
		linkChar := ac.activityConfig.linkChar
		if linkChar == "" {
			linkChar = defaultLinkChar
		}
...
func (ac *Activity) WithReturnKeyPrefix(keyPrefix string) {
	ac.activityConfig.returnKeyPrefix = keyPrefix
}
func (ac *Activity) WithKeyLinkCharacter(char string) {
	ac.activityConfig.linkChar = char
}

func (ac *Activity) mergeResponseWithId(resultMap map[string]any, actionFun action.Actor, requestParams any, retData any) map[string]any {
	if ac.Id == "" {
		return resultMap
	}

	if actionFun != nil {
		newParam, err := conv.ConvertForType(actionFun.ActionMeta().ArgumentType, requestParams)
		if err == nil {
	...
	idKey := ac.Id
	if ac.activityConfig.returnKeyPrefix != "" {
		idKey = ac.activityConfig.returnKeyPrefix + idKey
	}
...
func (ac *Activity) createResponse(keyPrefix string, actionFun action.Actor, requestParams any, retData any) map[string]any {
	...
	if len(retMap) == 0 {
		// 不是map类型，所以这里需要保存返回值
		actionKey := ac.getResponseStoreKey(keyPrefix)
		retMap[actionKey] = retData
	}
	resultMap = lo.Assign(resultMap, retMap)

	return ac.mergeResponseWithId(resultMap, actionFun, requestParams, retData)
}
...
func (ac *Activity) Execute(ctx context.Context, args map[string]any) (map[string]any, error) {
	...
	if ac.Control.DelayDuration > 0 {
		...
		_, err := goroutines.GoAsyncTimeout(time.Duration(ac.Control.Timeout)*time.Second, func(paramsIn ...any) (map[string]any, error) {
			_, err := ac.execThisAction(execCtx, inputParams)
			if err != nil {
				return nil, err
			}
			return nil, nil
	...
	return ac.execThisAction(execCtx, inputParams)
}

func (ac *Activity) execThisAction(execCtx context.Context, inputParams map[string]any) (map[string]any, error) {
	actionKey := ""
	paramKey := ""
	if ac.Control.CacheTime > 0 {
		actionKey = ac.getResponseStoreKey(ac.activityConfig.returnKeyPrefix)
	...
	retData, err := ac.executeByHookEvent(execCtx, LifecycleEventOnStart, inputParams)
	if err != nil {
		return inputParams, err
	}
	...
	resultMap := ac.createResponse(ac.activityConfig.returnKeyPrefix, actionFun, actionFunParam, execRetData)

	log.Print("baseFlow task return:", conv.String(resultMap), err)

	var onErr error

	retData, onErr = ac.executeByHookEvent(execCtx, LifecycleEventOnComplete, inputParams)
	if onErr == nil {
		if len(retData) > 0 {
	...
	if err != nil {
		retData, onErr = ac.executeByHookEvent(execCtx, LifecycleEventOnError, inputParams)
		if onErr == nil {
			if len(retData) > 0 {
				resultMap = tools.MergeNewArguments(resultMap, retData)
			}
	...
	retData, onErr = ac.executeByHookEvent(execCtx, LifecycleEventOnSuccess, inputParams)
	if onErr == nil {
		if len(retData) > 0 {
			resultMap = tools.MergeNewArguments(resultMap, retData)
		}
...
func (ac *Activity) execThisWhen(execCtx context.Context, inputParams map[string]any) (bool, map[string]any, error) {
	if len(ac.Control.WhenDepends) > 0 {
		...
		lo.ForEachWhile(ac.Control.WhenDepends, func(item *Activity, index int) bool {
			item.WithReturnKeyPrefix(ac.activityConfig.returnKeyPrefix)
			item.WithKeyLinkCharacter(ac.activityConfig.linkChar)
...
func (ac *Activity) executeByHookEvent(ctx context.Context, event LifecycleEvent, args map[string]any) (map[string]any, error) {
	if len(ac.Hooks) == 0 {
		return nil, nil
	}
	if oneAct, ok := ac.Hooks[event]; ok {
		...
		oneAct.WithReturnKeyPrefix(ac.activityConfig.returnKeyPrefix)
		oneAct.WithKeyLinkCharacter(ac.activityConfig.linkChar)
